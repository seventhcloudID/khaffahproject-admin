...
class TransaksiController extends Controller
{
    ...
    public function getListPeminatEdutrip(Request $request)
    {
        try {
            ...
            $data = $data->map(function ($item) {
                ...
                // Total pembayaran verified untuk Jurnal Transaksi / Laporan Keuangan
                $item->total_pembayaran_verified = (float) DB::table('pembayaran_transaksi_m')
    ...
    public function getLaporanKeuangan(Request $request)
    {
        try {
            ...
            $customList = $customData['data'] ?? [];
            $customList = is_array($customList) ? $customList : [];
            $komponenList = [];
            $customOnlyList = [];
            foreach ($customList as $item) {
                $itemArr = is_array($item) ? $item : (array) $item;
                $snap = $itemArr['snapshot_produk'] ?? null;
                if (is_string($snap)) {
                    $snap = json_decode($snap, true);
                }
                $snap = is_array($snap) ? $snap : [];
                $kategori = $snap['kategori_paket'] ?? $snap['nama_paket'] ?? '';
                $kategori = (string) $kategori;
                if (strpos($kategori, 'Komponen') === 0) {
                    $komponenList[] = $itemArr;
                } else {
                    $customOnlyList[] = $itemArr;
                }
            }
    ...
    /**
     * List transaksi REQUEST dikelompokkan per komponen (kategori_paket yang diawali "Komponen").
     * Untuk halaman admin "Transaksi Komponen": tiap grup berisi data transaksi per komponen (Hotel, dll).
     */
    public function getListTransaksiKomponenGrouped(Request $request)
    {
        try {
            $jenisRequest = JenisTransaksi::where('kode', 'REQUEST')->first();
            if (! $jenisRequest) {
                return response()->json([
                    'status'  => true,
                    'message' => 'List Transaksi Komponen berhasil diambil',
                    'data'    => ['groups' => []],
                ], 200);
            }
            ...
            $mapped = $data->map($mapItem);
            $groups = [];
            foreach ($mapped as $item) {
                $name = $item->nama_paket ?? '';
                if (strpos($name, 'Komponen') !== 0) {
                    continue;
                }
                if (! isset($groups[$name])) {
                    $groups[$name] = ['name' => $name, 'items' => []];
                }
                $groups[$name]['items'][] = (array) $item;
            }
            ...
            return response()->json([
                'status'  => false,
                'message' => 'Gagal mengambil data Transaksi Komponen: ' . $th->getMessage(),
            ], 500);
        }
    }
     ...
     *
     * Admin boleh mengubah ke Lunas tanpa harus ada record di pembayaran_transaksi_m
     * (mis. bayar offline / tunai / transfer yang dicatat manual).
     */
    public function updateStatusPembayaran(Request $request)
    {
        try {
            ...
            // Langsung update; tidak wajib ada record di pembayaran_transaksi_m
