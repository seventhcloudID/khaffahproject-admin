...
class PageContentController extends Controller
{
    ...
    public function updateAppSettings(Request $request)
    {
        $validated = $request->validate([
            ...
            'harga_visa' => 'nullable|numeric|min:0',
            'harga_visa_transportasi' => 'nullable|numeric|min:0',
        ]);
        $defaults = self::defaultAppSettings();
        $row = PageContent::where('slug', self::SLUG_APP_SETTINGS)->first();
        $existing = $row && is_array($row->content) ? $row->content : [];
        $content = array_merge($defaults, $existing);
        foreach ($validated as $key => $value) {
            if ($value !== null && $value !== '') {
                $content[$key] = $key === 'harga_visa' || $key === 'harga_visa_transportasi'
