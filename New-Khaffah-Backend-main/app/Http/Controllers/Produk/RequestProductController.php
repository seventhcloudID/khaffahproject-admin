...
class RequestProductController extends Controller
{
    public function store(Request $request)
    {
        $request->validate([
            ...
            'hotelRoomsDetail' => 'nullable|array',
            'hotelRoomsDetail.*.roomTypeName' => 'nullable|string|max:255',
            'hotelRoomsDetail.*.qty' => 'required|integer|min:1',
            'hotelRoomsDetail.*.hargaPerMalam' => 'required|numeric|min:0',
            ...
            'totalBiayaVisa' => 'nullable|numeric|min:0',
            'data_visa' => 'nullable|array',
            'totalBiayaBadalUmrah' => 'nullable|numeric|min:0',
            'data_badal_umrah' => 'nullable|array',
            'totalBiayaBadalHaji' => 'nullable|numeric|min:0',
            'data_badal_haji' => 'nullable|array',
            'totalBiayaTiketPesawat' => 'nullable|numeric|min:0',
            'data_tiket_pesawat' => 'nullable|array',
            'dibuat_sebagai_mitra' => 'nullable|boolean',
        ]);
        ...
        try {
            ...
            $snapshot = [
                ...
                'data_hotel' => array_filter([
                    ...
                    'room_details' => $request->input('hotelRoomsDetail'),
                ]),
                'data_keberangkatan' => [
            ...
            // Pesanan komponen (e.g. Komponen Hotel, Komponen Visa): hanya komponen saja
            $kategoriPaketRaw = $request->input('kategoriPaket');
            $isKomponenOnly = is_string($kategoriPaketRaw) && (str_starts_with($kategoriPaketRaw, 'Komponen') || $kategoriPaketRaw === 'Komponen Hotel');
            $isKomponenVisa = $kategoriPaketRaw === 'Komponen Visa';
            $isKomponenBadalUmrah = $kategoriPaketRaw === 'Komponen Badal Umrah';
            $isKomponenBadalHaji = $kategoriPaketRaw === 'Komponen Badal Haji';
            $isKomponenTiketPesawat = $kategoriPaketRaw === 'Komponen Tiket Pesawat';
            if ($isKomponenVisa) {
                $snapshot['data_visa'] = $request->input('data_visa', []);
            }
            if ($isKomponenBadalUmrah) {
                $snapshot['data_badal_umrah'] = $request->input('data_badal_umrah', []);
            }
            if ($isKomponenBadalHaji) {
                $snapshot['data_badal_haji'] = $request->input('data_badal_haji', []);
            }
            if ($isKomponenTiketPesawat) {
                $snapshot['data_tiket_pesawat'] = $request->input('data_tiket_pesawat', []);
            }
            $layananWajib = [];
            if (! $isKomponenOnly) {
            ...
            $roomDetails = $dataHotel['room_details'] ?? null;

            $totalHotel = 0;
            if (! empty($roomDetails) && is_array($roomDetails)) {
                foreach ($roomDetails as $rd) {
                    $qty = max((int) ($rd['qty'] ?? 0), 0);
                    $harga = (float) ($rd['hargaPerMalam'] ?? 0);
                    $totalHotel += $harga * $qty * $duration;
                }
            } else {
            ...
            if ($isKomponenVisa) {
                $totalBiayaOrder = (float) $request->input('totalBiayaVisa', 0);
            }
            if ($isKomponenBadalUmrah) {
                $totalBiayaOrder = (float) $request->input('totalBiayaBadalUmrah', 0);
            }
            if ($isKomponenBadalHaji) {
                $totalBiayaOrder = (float) $request->input('totalBiayaBadalHaji', 0);
            }
            if ($isKomponenTiketPesawat) {
                $totalBiayaOrder = (float) $request->input('totalBiayaTiketPesawat', 0);
            }
            ...
            $transaksi = Transaksi::create([
                ...
                'status_pembayaran_id' => StatusPembayaran::where('kode', 'BELUM_BAYAR')->value('id'),
