...
Route::prefix('/utility')->controller(UtilityController::class)->group(function () {
    ...
    Route::get('/bandara-tujuan', 'getBandaraTujuan');
...
// LA Umrah: hotel & maskapai bersumber dari Master Hotel / Master Maskapai. Dipakai isi-paket-kostumisasi & Komponen Hotel.
Route::prefix('la-umrah')->controller(\App\Http\Controllers\Master\LaUmrahOptionController::class)->group(function () {
    Route::get('/hotels', 'listHotelsPublic');       // Daftar hotel (Komponen Hotel + paket kostumisasi)
    Route::get('/maskapai', 'listMaskapaiPublic');
    Route::get('/hotels/{id}', 'showHotelPublic');   // Detail hotel untuk Komponen Hotel
});

// Public: Master Maskapai (sama dengan data di admin /Paket/Master-Maskapai). Untuk mitra komponen tiket.
Route::get('/master-maskapai/list', [\App\Http\Controllers\Master\MasterMaskapaiController::class, 'listPublic']);

Route::post('/webhook/moota', [WebhookController::class, 'handleMoota']);

// Public: master visa untuk halaman /mitra/komponen/visa (tanpa auth)
Route::get('/master-visa/list', [\App\Http\Controllers\Master\MasterVisaController::class, 'listPublic']);
// Public: master badal umrah untuk halaman /mitra/komponen/badal_umrah (tanpa auth)
Route::get('/master-badal-umrah/list', [\App\Http\Controllers\Master\MasterBadalUmrahController::class, 'listPublic']);
// Public: master badal haji untuk halaman /mitra/komponen/badal_haji (tanpa auth)
Route::get('/master-badal-haji/list', [\App\Http\Controllers\Master\MasterBadalHajiController::class, 'listPublic']);
...
Route::middleware(['auth:sanctum'])->group(function () {
    ...
    Route::middleware('role:user|mitra|superadmin')->group(function () {
        ...
        // --- Rekening perusahaan (untuk halaman pembayaran: /mitra/pesanan/[id]/pay & /account/orders/[id]/pay) ---
