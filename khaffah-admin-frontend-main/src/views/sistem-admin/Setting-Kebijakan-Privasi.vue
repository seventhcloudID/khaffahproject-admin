  ...
  <div class="columns">
    <Card class="column is-12" elevated radius="smooth" padding="md">
      <h1 class="text-xl font-medium mb-6 flex items-center gap-2">
        <i class="fas fa-shield-alt text-[#007b6f]"></i>
        Pengaturan Halaman Kebijakan Privasi
      </h1>
...
async function fetchData() {
  ...
  pesan.value = ''
  try {
    const res = await api.get('sistem-admin/setting-page/kebijakan-privasi')
...
async function simpan() {
  ...
  pesanSukses.value = false
  try {
    await api.put('sistem-admin/setting-page/kebijakan-privasi', { title: form.title, content: form.content })
    pesan.value = 'Kebijakan Privasi berhasil disimpan.'
