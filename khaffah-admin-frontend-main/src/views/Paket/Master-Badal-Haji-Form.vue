    ...
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
      <div class="flex items-center gap-4 mb-6">
        ...
        <h1 class="text-xl font-medium text-gray-900">
          {{ isEdit ? 'Edit Layanan Badal Haji' : 'Tambah Layanan Badal Haji' }}
        </h1>
      </div>
      ...
      <form @submit.prevent="saveData" class="space-y-6 max-w-2xl">
        ...
        <div>
          ...
          <textarea
            ...
            rows="2"
            class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-[#007b6f]"
            placeholder="Pelaksanaan ibadah Haji atas nama orang lain (sesuai syariat)"
          />
        </div>

        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label for="harga" class="block text-sm font-medium text-gray-700 mb-1">Harga per pax (IDR)</label>
            <input
              ...
              placeholder="0"
            />
            <p class="text-xs text-gray-500 mt-1">Isi 0 untuk menampilkan &quot;Hubungi kami&quot; di form mitra. Harga tampil di /mitra/komponen/badal_haji.</p>
          </div>
        ...
        <div>
          ...
          <p class="text-xs text-gray-500 mt-1">Hanya layanan aktif yang tampil di halaman /mitra/komponen/badal_haji.</p>
        </div>
...
async function loadDetail() {
  ...
  try {
    const res = await api.get(`/sistem-admin/master-badal-haji/${idParam.value}`)
    ...
    console.error('Gagal load detail Badal Haji', err)
  }
}
...
async function saveData() {
  ...
  try {
    ...
    if (!isEdit.value) {
      await api.post('/sistem-admin/master-badal-haji', payload)
    } else {
      await api.put(`/sistem-admin/master-badal-haji/${formData.value.id}`, payload)
    }
    router.push('/Paket/Master-Badal-Haji')
  } catch (err: any) {
