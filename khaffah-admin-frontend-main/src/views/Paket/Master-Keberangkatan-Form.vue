  ...
  <div class="columns">
    <Card class="column is-12" elevated radius="smooth" padding="md">
      <div class="flex items-center gap-4 mb-6">
        <RouterLink to="/Paket/Master-Keberangkatan" class="text-gray-600 hover:text-[#007b6f] flex items-center gap-1">
          <i class="fas fa-arrow-left"></i>
          <span>Kembali</span>
        </RouterLink>
        <h1 class="text-xl font-medium">{{ isEdit ? 'Edit Keberangkatan' : 'Tambah Keberangkatan' }}</h1>
      </div>
      ...
      <form @submit.prevent="saveData" class="space-y-6 max-w-2xl">
        <Field label="Kode (IATA)" id="kode" required>
          <input
            ...
            placeholder="Contoh: CGK, SUB, JOG"
          />
        </Field>
        ...
        <div class="flex gap-3 pt-4 border-t">
          ...
          <RouterLink to="/Paket/Master-Keberangkatan" class="px-4 py-2 border border-gray-300 rounded-md text-gray-700 hover:bg-gray-50">
            Batal
          </RouterLink>
        </div>
      </form>
...
async function loadDetail() {
  ...
  try {
    const res = await api.get(`/sistem-admin/master-keberangkatan/${idParam.value}`)
...
async function saveData() {
  ...
  try {
    ...
    if (!isEdit.value) {
      await api.post('/sistem-admin/master-keberangkatan', payload)
    } else {
      await api.put(`/sistem-admin/master-keberangkatan/${formData.value.id}`, payload)
    }
    router.push('/Paket/Master-Keberangkatan')
  } catch (err: any) {
