    ...
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
      <div class="flex items-center gap-4 mb-6">
        ...
        <h1 class="text-xl font-medium text-gray-900">
          {{ isEdit ? 'Edit Layanan Visa' : 'Tambah Layanan Visa' }}
        </h1>
      </div>
      ...
      <form @submit.prevent="saveData" class="space-y-6 max-w-2xl">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <input
              ...
              placeholder="Contoh: Visa & Transportasi"
            />
          </div>
          ...
          <div>
            ...
            <input
              ...
              id="slug"
              class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-[#007b6f]"
              placeholder="visa-transportasi (untuk tampilan di halaman mitra)"
            />
            <p class="text-xs text-gray-500 mt-1">Identifier unik. Kosongkan agar diisi otomatis dari nama layanan.</p>
          </div>
        </div>
        ...
        <div>
          ...
          <textarea
            ...
            rows="2"
            class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-[#007b6f]"
            placeholder="Pengurusan visa plus transportasi ke bandara"
          />
        </div>
        ...
        <div>
          ...
          <p class="text-xs text-gray-500 mt-1">Hanya layanan aktif yang tampil di halaman /mitra/komponen/visa.</p>
        </div>
...
async function loadDetail() {
  ...
  try {
    const res = await api.get(`/sistem-admin/master-visa/${idParam.value}`)
    ...
    console.error('Gagal load detail layanan visa', err)
  }
}
...
async function saveData() {
  ...
  try {
    ...
    if (!isEdit.value) {
      await api.post('/sistem-admin/master-visa', payload)
    } else {
      await api.put(`/sistem-admin/master-visa/${formData.value.id}`, payload)
    }
    router.push('/Paket/Master-Visa')
  } catch (err: any) {
