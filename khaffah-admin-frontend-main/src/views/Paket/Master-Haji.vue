<template>
  <div class="w-full max-w-full overflow-x-hidden">
    <Card class="w-full max-w-full px-4 sm:px-6 py-4 sm:py-6 box-border" elevated radius="smooth" padding="md">
      <h1 class="text-lg sm:text-xl font-medium mb-3 sm:mb-4">Master Paket Haji</h1>
        ...
        <template v-else>
          <div class="space-y-3">
            <div v-for="row in mobileCardsSlice" :key="row.id" class="bg-white border border-gray-200 rounded-xl p-4 shadow-sm">
              <h3 class="font-semibold text-gray-900 text-sm mb-2 truncate">{{ row.nama_paket }}</h3>
              <p class="text-sm text-gray-700 font-medium">{{ H.formatRupiah(row.biaya_per_pax) }}</p>
              <p class="text-xs text-gray-500 mt-1">Rating Hotel: {{ getAverageRating(row.akomodasi) }}</p>
              <p v-if="(row.akomodasi || []).length" class="text-xs text-gray-500 mt-1 line-clamp-2">
                {{ (row.akomodasi || []).slice(0, 3).map(h => `${h.kota}: ${h.hotel || h.nama_hotel || '-'}`).join(', ') }}
                <span v-if="(row.akomodasi || []).length > 3"> +{{ (row.akomodasi || []).length - 3 }} lainnya</span>
              </p>
              <p class="text-xs text-gray-500 mt-1">
                Waktu tunggu: {{ row.waktu_tunggu ? `${row.waktu_tunggu.min} - ${row.waktu_tunggu.max} Tahun` : '-' }}
              </p>
              <p v-if="(row.fasilitas_tambahan || []).length" class="text-xs text-gray-500">Fasilitas: {{ row.fasilitas_tambahan.length }} item</p>
        ...
        >
          <template #header>
            <div class="flex flex-wrap gap-2 items-center pt-2">
              <Button color="success" @click="exportExcel()" outlined icon="fas fa-file-excel" :hide-text-on-mobile="true" class="min-w-0">
                Export To Excel
              </Button>
            </div>
          </template>
          ...
          <Column header="Harga" style="min-width: 150px">
            <template #body="slotProps">
              {{ H.formatRupiah(slotProps.data.biaya_per_pax) }}
            </template>
          </Column>
          <Column header="Rating Hotel" style="width: 100px">
            <template #body="slotProps">
              {{ getAverageRating(slotProps.data.akomodasi) }}
            </template>
          </Column>
          <Column header="Daftar Hotel" style="min-width: 180px">
            <template #body="slotProps">
              <div class="whitespace-pre-line text-sm max-w-xs">
                <div
                  v-for="(hotel, idx) in (slotProps.data.akomodasi || []).slice(0, 3)"
                  :key="idx"
                  class="mb-1 last:mb-0"
                >
                  <span class="text-gray-700">{{ hotel.kota }}: {{ hotel.hotel }}</span>
                </div>
                <span v-if="(slotProps.data.akomodasi || []).length > 3" class="text-gray-500 text-xs">
                  +{{ (slotProps.data.akomodasi || []).length - 3 }} lainnya
                </span>
                <span v-if="!(slotProps.data.akomodasi || []).length" class="text-gray-400">-</span>
              </div>
            </template>
          </Column>
          <Column header="Waktu Tunggu" style="min-width: 140px">
            <template #body="slotProps">
              <span v-if="slotProps.data.waktu_tunggu">
                {{ slotProps.data.waktu_tunggu.min }} - {{ slotProps.data.waktu_tunggu.max }} Tahun
              </span>
              <span v-else class="text-gray-400">-</span>
            </template>
          </Column>
          <Column header="Fasilitas" style="min-width: 120px">
            <template #body="slotProps">
              <span v-if="slotProps.data.fasilitas_tambahan?.length">
                {{ slotProps.data.fasilitas_tambahan.length }} item
              </span>
              <span v-else class="text-gray-400">-</span>
            </template>
          </Column>
...
const deleteData = async (data: any) => {
  ...
  try {
    await api.delete(`/sistem-admin/paket-haji/delete-paket-haji/${data.id}`)
    alert('Paket haji berhasil dihapus')
    fetchData()
  } catch (error) {
...
const fetchData = async () => {
  try {
    ...
    const response = await api.get('/sistem-admin/paket-haji/get-paket-haji?show_all=1')
...
const exportExcel = () => {
  ...
  const tglExport = H.formatDate(new Date(), 'DD-MM-YYYY')
  ...
  sheetData.push([`Export: ${tglExport}`])
  sheetData.push([])
  ...
  dataSource.value.forEach((e: any) => {
    const akomodasi = e.akomodasi ?? []
    const daftarHotel = akomodasi
      .map((hotel: any) => `${hotel.kota ?? ''}: ${hotel.hotel ?? hotel.nama_hotel ?? '-'}`)
      .join('; ')
    ...
    const wt = e.waktu_tunggu ?? {}
    const waktuTunggu = `${wt.min ?? 0} - ${wt.max ?? 0} Tahun`
  ...
  const fileName = `Master Paket Haji ${tglExport}.xlsx`
