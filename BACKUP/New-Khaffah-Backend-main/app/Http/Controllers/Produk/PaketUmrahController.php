...
class PaketUmrahController extends Controller
{
        ...
        try {
            ...
            foreach ($pakets as $p) {
                ...
                $data[] = [
                    ...
                    'url_foto' => $foto ?? '',
                    'destinasi' => $destinasi,
                    'harga_termurah' => $p->harga_termurah !== null ? (float) $p->harga_termurah : 0,
                    'harga_termahal' => $p->harga_termahal !== null ? (float) $p->harga_termahal : null,
                    'harga_asli_termurah' => null,
                    'harga_asli_termahal' => null,
                    'persen_potongan_mitra' => 0,
                ];
            }
    ...
    public function getPaketUmrah(Request $request)
    {
        ...
        try {
            $pum = DB::table('paket_umrah_m as pum')
                ->leftJoin('musim_m as mm', 'mm.id', '=', 'pum.musim_id')
            ...
            $fotoPaket = DB::table('paket_umrah_foto_m')
                ...
                ->orderBy('urutan')
                ->get(['url_foto', 'urutan'])
                ->map(fn ($r) => ['url_foto' => $r->url_foto, 'urutan' => (int) $r->urutan])
                ->all();
            ...
            $destinasi = DB::table('paket_umrah_destinasi_t as pudt')
                ...
                ->where('pudt.paket_umrah_id', $id)
                ->where('pudt.is_active', true)
                ->select('k.nama_kota', 'pudt.durasi')
                ->get()
                ->map(fn ($d) => ['nama_kota' => $d->nama_kota ?? '', 'durasi' => (int) ($d->durasi ?? 0)])
                ->all();
            ...
            $tipeRows = DB::table('paket_umrah_tipe_m as put')
            ...
            $tipePaketUmrah = $tipeRows->map(function ($t) {
                return [
            ...
            $data = [
                ...
                // jumlah_pax dikirim sebagai SISA pax, bukan kapasitas mentah
                'jumlah_pax' => $sisaPax,
                'harga_termurah' => $pum->harga_termurah !== null ? (string) $pum->harga_termurah : '0',
                'hotel' => $hotel,
                'maskapai' => $maskapai,
    ...
    public function getPaketUmrahId(Request $request, int $id)
    {
        ...
        $request->merge(['paket_umrah_id' => $id]);
